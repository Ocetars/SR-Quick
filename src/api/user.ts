...
export const getUserType = async () => {
  const res = await Smartrequest<ApiWrapper<UserTypeData>>(
    "/api/v1/user/type",
    {
      method: "GET",
    }
...
export const bindUserUid = async (payload: { uid: string }) => {
  const res = await Smartrequest<ApiWrapper<BindUserResponseData>>(
    "/api/v1/user/bind",
    {
      method: "POST",
      data: payload,
...
export const refreshPlayerByUid = async (uid: string) => {
  const res = await Smartrequest<RefreshPlayerResponse>(
    `/api/v1/player/${uid}`,
    {
      method: "GET",
    }
...
export const getPlayerSummary = async (uid: string) => {
  const res = await Smartrequest<PlayerSummaryResponse>(
...
export const getCharacterDetail = async (params: {
  ...
  const res = await Smartrequest<import("@/types/mihomo").CharacterInfo>(
