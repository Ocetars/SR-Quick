...
export default function Detail() {
  ...
  // 用户状态
  const { user, isLoggedIn, currentAccount, gameAccounts, addGameAccount } =
    useUser();
  ...
  const handleSyncData = async () => {
    ...
    try {
      // 检查用户是否已绑定此UID
      const hasAccount = gameAccounts.some(
        (account) => account.uid === playerInfo.uid,
      );
  ...
  const isCurrentUserAccount = () => {
    return (
      isLoggedIn &&
      playerInfo &&
      gameAccounts.some((account) => account.uid === playerInfo.uid)
    );
  };
          ...
          <View className="detail__card">
            ...
            <View className="detail__player-basic">
              <View className="detail__avatar-section">
                ...
                <View className="detail__avatar-info">
                  <Text className="detail__avatar-name">
                    {playerInfo.avatar?.name || "未知角色"}
                  </Text>
                </View>
              </View>
              <View className="detail__basic-info">
                ...
                <View className="detail__info-row">
                  ...
                  <Text className="detail__value">
                    {playerInfo.world_level}
                  </Text>
                </View>
                <View className="detail__info-row">
                  <Text className="detail__label">好友数量:</Text>
                  <Text className="detail__value">
                    {playerInfo.friend_count}
                  </Text>
                </View>
              </View>
            </View>
            {playerInfo.signature && (
              <View className="detail__signature">
                ...
                <Text className="detail__signature-text">
                  "{playerInfo.signature}"
                </Text>
              </View>
            )}
          ...
          <View className="detail__card">
            ...
            <View className="detail__memory-data">
              <View className="detail__info-row">
                ...
                <Text className="detail__value">
                  {playerInfo.space_info?.memory_data?.level || 0}
                </Text>
              </View>
              <View className="detail__info-row">
                <Text className="detail__label">混沌回忆等级:</Text>
                <Text className="detail__value">
                  {playerInfo.space_info?.memory_data?.chaos_level || 0}
                </Text>
              </View>
              <View className="detail__info-row">
                <Text className="detail__label">混沌回忆星数:</Text>
                <Text className="detail__value">
                  {playerInfo.space_info?.memory_data?.chaos_star_count || 0}
                </Text>
              </View>
            </View>
          </View>
            ...
            <View className="detail__card">
              ...
              <View className="detail__stats-grid">
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.avatar_count}
                  </Text>
                  <Text className="detail__stat-label">角色</Text>
                </View>
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.light_cone_count}
                  </Text>
                  <Text className="detail__stat-label">光锥</Text>
                </View>
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.relic_count}
                  </Text>
                  <Text className="detail__stat-label">遗器</Text>
                </View>
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.achievement_count}
                  </Text>
                  <Text className="detail__stat-label">成就</Text>
                </View>
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.book_count}
                  </Text>
                  <Text className="detail__stat-label">图鉴</Text>
                </View>
                <View className="detail__stat-item">
                  <Text className="detail__stat-value">
                    {playerInfo.space_info.music_count}
                  </Text>
