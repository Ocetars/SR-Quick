  ...
  useEffect(() => {
    if (characters.length > 0) {
      setStats({
        ...
        lastSyncTimeFormatted: lastSyncTime ? userUtils.formatSyncTime(lastSyncTime) : '未同步'
      })
    }
      ...
      <View className='user'>
        <View className='user__login-container'>
          <View className='user__login-card'>
            ...
            <Text className='user__login-subtitle'>登录后可以云端保存角色面板数据</Text>
    ...
    <View className='user'>
      ...
      <View className='user__profile-card'>
        <View className='user__avatar-section'>
          ...
          <View className='user__info'>
            <Text className='user__nickname'>{user?.nickname || '星穹旅者'}</Text>
            <Text className='user__uid'>
              {currentAccount ? `主账号：${currentAccount.uid}` : '未添加游戏账号'}
            </Text>
            {currentAccount && (
              <Text className='user__level'>
                Lv.{currentAccount.level || 0} | 世界等级 {currentAccount.world_level || 0}
              </Text>
            )}
          </View>
        </View>
      ...
      <View className='user__stats-card'>
        ...
        <View className='user__sync-info'>
          <Text className='user__sync-text'>最后同步：{stats.lastSyncTimeFormatted}</Text>
        </View>
      </View>
