        ...
        try {
          ...
          if (storedUserInfo) {
            try {
              ...
              const currentAccount = userUtils.getPrimaryGameAccount(
      ...
      setPrimaryAccount: async (uid: string) => {
        try {
          ...
          const newCurrentAccount =
            updatedAccounts.find((account) => account.uid === uid) ||
...
export const useUserDataStore = create<UserDataState>((set, get) => ({
  ...
  syncCharacters: async (uid: string, forceUpdate: boolean = false) => {
    ...
    try {
      const result = await userDataAPI.syncCharacters({
        uid,
  ...
  deleteCharacter: async (uid: string, characterId: string) => {
    try {
      ...
      const updatedCharacters = state.characters.filter(
        (char) => !(char.uid === uid && char.character_id === characterId),
      );
